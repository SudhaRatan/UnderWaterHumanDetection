...
function preprocessImage(image) {
  ...
  let tensor = tf.browser.fromPixels(image)
      // .resizeNearestNeighbor([640, 640]) // Resize the image to 640x640
      // .toFloat()
...
const ButtonHandler = ({ imageRef, cameraRef, videoRef }) => {
  ...
  return (
    <div className="btn-container">
      ...
      />
      <button
      className="bg-black hover:bg-white text-white hover:text-black border-2 border-[black] px-4 py-2 m-2 rounded-full"
        onClick={() => {
          // if not streaming
